...
import * as moment from 'moment';
  ...
  export class AuthService {
    ...
    async resetPassword(resetPasswordDto: RequestPasswordResetDto): Promise<void> {
      const {email, password} = resetPasswordDto;  
      const isOtpValid = await prisma.otp.findFirst({where : {email}});
      if(!isOtpValid){
        throw new BadRequestException('Whoops, seems like you are yet to request an otp')
      }
