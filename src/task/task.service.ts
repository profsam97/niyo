...
export class TaskService {
  ...
  async getTaskById(userId: number, id: string): Promise<ITask> {
    const taskId: number = parseInt(id);

    return await prisma.task.findUnique({
      where: { id: taskId, owner: userId },
    });
  }
  async updateTask(
    userId: number,
    ...
    const { name, description } = updateTask;
    ...
    if (task.owner !== userId) {
      throw new UnauthorizedException(
        'Sorry you are not authorized to update this task',
      );
    }
    if (name === '' && description === '') {
      throw new BadRequestException(
        'You need to provide a name or description in order to update the task',
      );
    }
  ...
  async deleteTask(userId: number, taskId: string): Promise<string> {
    ...
    if (task.owner !== userId) {
      throw new UnauthorizedException(
        'Oops, you are not authorized to delete this task',
      );
    }
